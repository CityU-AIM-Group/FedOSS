...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.fc = MixLinear(512 * block.expansion, num_classes)        
    ...
    def forward(self, x, targets=None, mode='softmax_threshold',fixed = False, alpha=0.8, is_detach = True, temperature=1.0, is_normalized=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        x = torch.flatten(x, 1)
        #print(x.shape)
        x = self.fc(x, targets, mode, fixed, alpha, is_detach, temperature, is_normalized)
...
if __name__=='__main__':
    ...
    y = net(x, targets, mode='fedosr', fixed = True, alpha=0.8, is_detach = False, temperature = 1.0, is_normalized = False)
    print(y)
